...
void Config::Read(char* fname){
  ...
  while(!feof(f)){
    ...
    if(sscanf(line,"Window.Name: %s",s)==1)     {strcpy(WND.Name,line+13); continue;}
    if(sscanf(line,"FoV: %lf",v+0)==1)          {WND.FoV=v[0]; continue;}
    if(sscanf(line,"TEX.Sky: %s",s)==1) {strcpy(TEX.gSky,s); continue;}
    if(sscanf(line,"TEX.Map: %s",s)==1) {strcpy(TEX.gMap,s); continue;}
    ...
    if(sscanf(line,"TEX.Det: %s",s)==1)         
    {
       ...
       printf("TEX.Det -> %s\n=%s",s,TEX.gDet); 
       continue;}
    if(sscanf(line,"TEX.Compass: %s",s)==1)     {strcpy(TEX.gCompas,s); continue;}
    if(sscanf(line,"DEM.File: %s",s)==1)        {strcpy(DEM.File,s); continue;}
    if(sscanf(line,"DEM.Format: %s",s)==1)  DEM.Format.Read(s);
    if(sscanf(line,"SAVE.ScrShot: %s",s)==1){strcpy(SAVE.ShotName,s); continue;}    
    if(sscanf(line,"SAVE.FluxFile: %s",s)==1){strcpy(SAVE.FluxFile,s); continue;}    
    if(sscanf(line,"SAVE.FluxPlot: %s",s)==1){strcpy(SAVE.FluxPlot,s); continue;}    
  }
  fclose(f);
  printf("Dem file=%s\n",DEM.File);
  printf("TXmax=%2.5f\n",DET.TXmax);
...
void Config::Print(){
  ...
  printf("TEX.gPhoto = \"%s\"\n",TEX.gPhoto?TEX.gPhoto:"NULL");
  printf("TEX.gMap = \"%s\"\n",TEX.gMap?TEX.gMap:"NULL");
  printf("TEX.gCompas = \"%s\"\n",TEX.gCompas?TEX.gCompas:"NULL");
  printf("TEX.gDet = \"%s\"\n",TEX.gDet?TEX.gDet:"NULL");
  printf("TEX.gSky = \"%s\"\n",TEX.gSky?TEX.gSky:"NULL");
